...
from chains import chain_extrat , chain_reflect
...
def draft_node(state: MessagesState):
    ...
    response = chain_extrat.invoke({"messages": state["messages"]})
    return {"messages": [response]}


def revise_node(state: MessagesState):
    """Revise the answer based on tool results."""
    response = chain_reflect.invoke({"messages": state["messages"]})
    return {"messages": [response]}
